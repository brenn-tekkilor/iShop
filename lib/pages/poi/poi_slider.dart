...
class POISlider extends StatelessWidget implements PreferredSizeWidget {
  const POISlider({Key key}) : super(key: key);
    ...
    return ValueListenableBuilder(
      builder: (BuildContext context, double radius, _) {
        ...
        return Slider(
          ...
          divisions: divisions,
          value: startValue,
          label: Calculator.kilometerToMiles(data.streamingRadius)
                  .toInt()
                  .toString() +
              ' mi.',
          onChanged: (val) {
            if (val is double &&
              ...
              data.streamingRadius = convertValueToRadius(startValue);
            }
          },
        );
      },
      valueListenable: data.streamingRadiusListenable,
    );
  }
}
